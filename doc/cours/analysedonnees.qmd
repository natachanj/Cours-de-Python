...
Un **DataFrame** est une table composée de lignes et de colonnes, similaire à une feuille de calcul Excel ou une base de données SQL. L'une des façons les plus courantes de créer un DataFrame est d'utiliser un **dictionnaire Python**.
...
- Il permet de manipuler facilement **des données hétérogènes**.
- Il est conçu pour des **opérations analytiques rapides et intuitives**.
- Il facilite la conversion et l'import/export de données dans divers formats (CSV, Excel, SQL, etc.).

### Accéder aux informations du DataFrame

Une fois le DataFrame créé, il est essentiel d'explorer ses informations :

```python
print(df.head())  # Afficher les premières lignes
df.info()  # Afficher un résumé des données
...
- Aide à **comprendre la structure des données** avant d'appliquer des transformations.

**Exercice :**
- Créez un DataFrame à partir d'un dictionnaire contenant au moins 4 colonnes.
- Affichez les informations de base du DataFrame.
