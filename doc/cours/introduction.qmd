...
description: "[Slides](introduction-python-slides.html)"
...
Les listes sont des structures de données fondamentales en Python. Elles permettent de stocker plusieurs éléments dans un ordre spécifique et sont très utilisées pour manipuler des collections de données.

---

### Qu'est-ce qu'une liste ?

Une liste est une collection ordonnée et modifiable d'éléments. Ces éléments peuvent être de différents types (nombres, chaînes de caractères, booléens, etc.).
...
1. **Ordonnées** : Les éléments conservent leur position dans l'ordre dans lequel ils ont été ajoutés.
2. **Modifiables** : Les listes peuvent être modifiées après leur création (ajout, suppression, modification d'éléments).
3. **Hétérogènes** : Une liste peut contenir des éléments de différents types.

---

### Opérations sur les listes

1. **Indexation** : Chaque élément de la liste a une position (index), à partir de 0 pour le premier élément.
2. **Slicing (Découpage)** : Permet d'accéder à une sous-partie de la liste en utilisant des indices.
3. **Concaténation** : Deux listes peuvent être combinées pour former une nouvelle liste.
4. **Répétition** : Une liste peut être répétée plusieurs fois.

---

### Méthodes communes des listes

1. **Ajouter des éléments** : Avec les méthodes comme `append()` pour ajouter un élément à la fin ou `insert()` pour l’ajouter à une position spécifique.
2. **Supprimer des éléments** : Avec des méthodes comme `remove()` pour supprimer une valeur spécifique ou `pop()` pour supprimer un élément à un index donné.
3. **Rechercher des éléments** : La méthode `index()` permet de trouver la position d'un élément.
4. **Trier et inverser** : Avec les méthodes `sort()` pour trier et `reverse()` pour inverser l'ordre des éléments.

---

### Parcourir une liste

Les listes peuvent être parcourues élément par élément, généralement à l'aide de boucles.

---

### Les listes imbriquées

Les listes peuvent contenir d'autres listes, ce qui permet de créer des structures de données en deux dimensions ou plus.

---

### Résumé des principales méthodes de liste
...
| `pop(i)`      | Supprime l'élément à l'index `i` et le retourne.|
...
Les listes sont un outil puissant en Python et constituent un élément de base pour manipuler et organiser des données dans vos programmes.
